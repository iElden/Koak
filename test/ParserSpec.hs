...
parseCharTest = describe "parseCharTest (UT)" $ do
    ...
    it "nothing on parameter" $
        runParser (parseChar []) "ade" `shouldBe` (Nothing, "ade")
    it "something wrong to parse" $
        runParser (parseChar "ade") "bte" `shouldBe` (Nothing, "bte")
...
parseCharBlackListTest = describe "parseCharBlackListTest (UT)" $ do
    ...
    it "blacklist first character" $
        runParser (parseCharBlackList (['0'..'9'] ++ [' '])) "1231 a" `shouldBe` (Nothing, "1231 a")
...
parseCharSequenceTest = describe "parseCharSequenceTest (UT)" $ do
    ...
    it "choked patt" $
        runParser (parseCharSequence "add") "ad d" `shouldBe` (Nothing, " d")
    it "nothing nothing" $
...
parseDigitTest = describe "parseDigitTest (UT)" $ do
    ...
    it "not a digit" $
        runParser parseDigit "a0" `shouldBe` (Nothing, "a0")
...
parseAlphaTest = describe "parseAlphaTest (UT)" $ do
    ...
    it "parse not alpha" $
        runParser parseAlpha "8" `shouldBe` (Nothing, "8")
...
parseWhiteSpaceTest = describe "parseWhiteSpaceTest (UT)" $ do
    ...
    it "parse nothing matching it" $
        runParser parseWhiteSpace "a" `shouldBe` (Nothing, "a")

parseIntegerTest :: Spec
parseIntegerTest = describe "parseIntegerTest (UT)" $ do
    it "Not a Integer" $
        runParser parseInteger "oui" `shouldBe` (Nothing, "oui")
    it "integer" $
    ...
    it "Pattern : N." $
        runParser parseInteger "1." `shouldBe` ((Just $ Nbr 1),  ".")
    it "Pattern : .N" $
        runParser parseInteger ".1" `shouldBe` (Nothing, ".1")
    it "Pattern : N.N" $
...
parseDoubleTest = describe "parseDoubleTest (UT)" $ do
    it "Not a number" $
        runParser parseDouble "oui" `shouldBe` (Nothing, "oui")
    it "Pattern : N.X" $
...
parseIdentifierTest = describe "parseIdentifierTest (UT)" $ do
    ...
    it "parse not identifier" $
        runParser parseIdentifier "0azeza" `shouldBe` (Nothing, "0azeza")
    it "parse identifier" $
    ...
    it "parse space + identifier" $
        runParser parseIdentifier " Adding" `shouldBe` (Nothing, " Adding")
...
parseTypedIdentifierTest = describe "parseTypedIdentifierTest (FT)" $ do
    ...
    it "incorrect pattern typed identifier" $
        runParser parseTypedIdentifier "voi:" `shouldBe` (Nothing, "voi:")
    it "incorrect pattern typed identifier 2" $
        runParser parseTypedIdentifier ": void" `shouldBe` (Nothing, ": void")
...
parseBinExprTest = describe "parseBinExprTest (FT)" $ do
    ...
    it "parse not correct binary" $
        runParser (parseBinExpr 0) "<4" `shouldBe` (Nothing, "<4")
    it "parse not correct binary 2" $
        runParser (parseBinExpr 0) "4<" `shouldBe` ((Just $ Unary [] $ Nbr 4), "<")
    it "parse t=." $
